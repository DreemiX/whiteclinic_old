...
class DynamicGoogleMaps extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    protected function safe_register_controls()
    {
        ...
        $this->add_control('notice_new_google_api', ['content' => esc_html__('Important Update on Google Maps API: The new API introduces a new default color palette and improved map experiences and usability. As of February 2025, the old APIs will no longer be available, and all services will be automatically transitioned to the new API. The activation of the new APIs does not allow map customization. We are looking for a possible solution with the Google Maps API documentation to make it feasible in the future.', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::NOTICE, 'notice_type' => 'warning']);
    ...
    protected function safe_render()
    {
        ...
        if (\Elementor\Plugin::$instance->editor->is_edit_mode()) {
            if (!empty($settings['new_api']) && isset($settings['style_select']) && \in_array($settings['style_select'], ['prestyle', 'custom'], \true)) {
                Helper::notice(\false, esc_html__('Due to changes in the Google Maps APIs, the style is no longer supported. Google no longer allows the insertion of styles for security reasons, and we are awaiting further information from them to provide a solution.', 'dynamic-content-for-elementor'), 'elementor-alert-danger');
            }
