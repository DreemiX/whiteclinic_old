...
if ( ! class_exists( 'CBI_Widget_Areas' ) ) {

	class CBI_Widget_Areas {
		...
		public function __construct( $widget_areas = array() ) {
			...
			add_action( 'wp_ajax_cbi_delete_widget_area', array( $this, 'cbi_delete_custom_widget_area' ) );
			add_action( 'wp_ajax_cbi_add_widget_area', array( $this, 'cbi_add_custom_widget_area' ) );
		...
		public function cbi_add_custom_widget_area() {
			...
			if ( $screen->id == 'widgets' ) {
				$content = '
                    <div id="sl-add-widget-template">
                                        ...
                                        <div id="widget_informer"></div>
                                            <div class="sl-add-widget-template">
                                                <div class="sidebar-name">
                                                    <h3>' . esc_html__( 'Create a Sidebar', 'clientica-integrator' ) . '<span class="spinner"></span></h3>
                                                </div>
						...
						'clientica-integrator' ) . '" placeholder="' . esc_attr__( 'Name', 'clientica-integrator' ) . '" />
                                                </div>
		...
		function cbi_delete_custom_widget_area() {
